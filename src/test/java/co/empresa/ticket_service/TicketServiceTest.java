...
void generateTicket_createsNewTicket() {
    ...
    assertThat(response.getTicketTypeId()).isEqualTo("type-001");
    assertThat(response.getTicketTypeName()).isEqualTo("General");
...
void generateTicket_handlesConcurrentDuplicate() {
    ...
    when(qrService.generateQrBase64(anyString()))
...
void validateTicket_activeTicket_returnsValidAndMarksUsed() {
    when(ticketRepo.findByQrToken("qr-token-001"))
            .thenReturn(Optional.of(activeTicket));
    ...
    assertThat(activeTicket.getStatus())
            .isEqualTo(Ticket.TicketStatus.USED);

    assertThat(activeTicket.getUsedAt())
            .isNotNull();
...
@Test
@DisplayName("validateTicket: boleta USED → valid=false")
...
@Test
@DisplayName("validateTicket: QR inexistente → valid=false")
void validateTicket_unknownToken_returnsInvalid() {
    when(ticketRepo.findByQrToken("token-falso"))
            .thenReturn(Optional.empty());
...
@Test
@DisplayName("validateTicket: boleta USED incluye fecha de uso")
...
void getById_ownTicket_returnsResponse() {
    when(ticketRepo.findById("ticket-001"))
            .thenReturn(Optional.of(activeTicket));

    TicketResponse response =
            ticketService.getById("ticket-001", "buyer-001");
...
void getById_notFound_throws404() {
    when(ticketRepo.findById("no-existe"))
            .thenReturn(Optional.empty());

    assertThatThrownBy(() ->
            ticketService.getById("no-existe", "buyer-001"))
...
void getById_differentBuyer_throws403() {
    ...
    assertThatThrownBy(() ->
            ticketService.getById("ticket-001", "otro-buyer"))
            .isInstanceOf(ResponseStatusException.class)
            .satisfies(ex -> assertThat(
                    ((ResponseStatusException) ex)
                            .getStatusCode()
